...
import { ReferralPartnerHeroVisual } from "@/components/referral-partner/referral-partner-hero-visual"

const trustPoints = ["Commission on referrals", "Real-time tracking", "Dedicated support"]

interface ReferralPartnerHeroSectionProps {
  onLoginOpen: () => void
}

export function ReferralPartnerHeroSection({ onLoginOpen }: ReferralPartnerHeroSectionProps) {
  return (
    <section
      id="hero"
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 lg:gap-x-6 xl:gap-x-8 items-center">
          ...
          >
            ...
            <h1 className="font-display text-[2rem] sm:text-4xl lg:text-[2.5rem] xl:text-[2.85rem] font-semibold leading-[1.08] tracking-tight mb-4">
              <span className="text-[#1F4068]">Grow with Manavizha</span>
              <br />
              <span className="text-brand-gold">earn with every match</span>
            </h1>

            <p className="text-base sm:text-lg text-gray-600 leading-relaxed max-w-lg mx-auto lg:mx-0 mb-8">
              Help families find meaningful connections while building a rewarding referral business
              with transparent commissions and an easy-to-use partner dashboard.
            </p>
      ...
      >
        <div className="flex items-center justify-center gap-3 px-4 text-sm">
          ...
          <span className="text-center font-medium text-[#1F4068]/70">
            Trusted by partners across Tamil Nadu and beyond.
          </span>
          <Heart className="h-3.5 w-3.5 fill-[#e87898]/50 text-[#e87898]/50" aria-hidden />
