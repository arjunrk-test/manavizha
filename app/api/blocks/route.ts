...
export async function GET(request: Request) {
    try {
        ...
        const { data, error } = await supabaseAdmin.from('blocked_profiles').select('blocked_user_id').eq('user_id', userId)
...
export async function POST(request: Request) {
    try {
        ...
        const { error } = await supabaseAdmin.from('blocked_profiles').insert({
            user_id: userId,
...
export async function DELETE(request: Request) {
    try {
        ...
        const { error } = await supabaseAdmin.from('blocked_profiles').delete().eq('user_id', userId).eq('blocked_user_id', targetUserId)
